-- ============================================================
-- SCHEMA COMPLETO + MIGRACIÓN — Certifik PLD (CNBV 2026)
-- Pega TODO este contenido en:
-- https://supabase.com/dashboard/project/xuwestiepwhlbdnwyblb/sql/new
-- Haz clic en RUN — debe decir "Success"
-- ============================================================
...
create index if not exists documents_is_global_idx on public.documents(is_global) where is_global = true;
...
create index if not exists document_embeddings_user_id_idx on public.document_embeddings(user_id);
...
-- ========================
-- RPC: Vector search (incluye docs globales)
-- ========================
...
language sql stable as $$
  select
    de.id, de.document_id, de.content, de.metadata,
...
for select using (
  auth.uid() = user_id
  or exists (select 1 from public.documents d where d.id = document_id and d.is_global = true)
);

drop policy if exists "embeddings_insert_own" on public.document_embeddings;
create policy "embeddings_insert_own" on public.document_embeddings
for insert with check (auth.uid() = user_id);
